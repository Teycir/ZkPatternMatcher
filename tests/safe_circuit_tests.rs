...
fn test_no_false_positives_on_safe_multiplier() {
    ...
    let matches = matcher
        .scan_file(Path::new("tests/safe_circuits/safe_multiplier.circom"))
...
fn test_no_false_positives_on_safe_merkle() {
    ...
    let matches = matcher
        .scan_file(Path::new("tests/safe_circuits/safe_merkle.circom"))
        .expect("Failed to scan file");
...
fn test_safe_circuits_batch() {
    ...
    for circuit in safe_circuits {
        ...
        assert_eq!(
            critical_or_high, 0,
            "Safe circuit {} should not trigger critical/high findings",
            circuit
        );
    }
}
