...
fn test_batch_scan_multiple_files() {
    ...
    let output = Command::new("sh")
        .arg("-c")
        .arg("find tests/real_vulnerabilities -name '*.circom' | \
...
fn test_ci_pipeline_simulation() {
    ...
    assert!(!output.status.success(), "CI should fail on vulnerabilities");
...
fn test_pattern_library_discovery() {
    ...
    let pattern_lines: Vec<&str> = lines.iter()
        .filter(|l| l.contains("[Critical]") || l.contains("[High]"))
        .copied()
        .collect();
    
    assert!(pattern_lines.len() >= 3, "Should have multiple high-severity patterns");
}
...
fn test_large_file_handling() {
    ...
    assert!(output.status.code().is_some(), "Should complete without crash");
}
